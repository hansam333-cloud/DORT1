# Detail Page Example

## Input Summary

```text
Product title: [피지컬핏] 쿨링 머슬 반팔티
Category: Short sleeve t-shirt
Expected selling price: 37,900 KRW
Colors: Black, White, Charcoal, Navy
Sizes: M, L, XL
Material: Soft cotton blend
Product intended fit: Athletic fit
Product registration record: DORT-TEE-001-BK registration record, GREEN
Season: Spring / Summer / Early fall

Model height: 183cm
Model weight: 78kg
Model usual size: 105 / L
Worn size: L
Fit notes: Shoulder and chest look defined, waist does not look bulky, length lands around the hip line.

Main body/proportion benefit:
Upper body looks wider and cleaner without feeling like gym wear.

Customer anxiety:
Customers may worry that the shirt is too tight, too short, or too gym-like.
```

## Output

# Detail Page Result

## Product Title

```text
[피지컬핏] 쿨링 머슬 반팔티
```

## One-Line Positioning

```text
운동한 체형은 자연스럽게 살리고, 과하게 붙는 느낌은 줄인 데일리 피지컬핏 반팔티.
```

## Top Purchase Hooks

- 상체 라인이 깔끔하게 살아나는 애슬레틱핏
- 팔은 자연스럽게 차 보이고, 허리는 부해 보이지 않는 실루엣
- 데님, 슬랙스, 쇼츠에 모두 어울리는 데일리 기본티

## Product Summary

`[피지컬핏] 쿨링 머슬 반팔티`는 운동한 체형을 자연스럽게 보여주기 위한 데일리 티셔츠입니다. 어깨와 가슴 라인은 살리고, 허리와 몸통은 과하게 퍼지지 않도록 잡아 깔끔한 상체 밸런스를 만듭니다.

과하게 붙는 헬스핏이 부담스러운 고객에게 적합하며, 일상 코디에서도 편하게 입을 수 있는 피지컬핏 기본 아이템입니다.

## DORT FIT POINT

### 1. 어깨가 안정적으로 넓어 보이는 라인

어깨선이 애매하게 처지지 않아 상체 프레임이 깔끔하게 잡힙니다.

### 2. 팔 라인이 자연스럽게 차 보이는 소매통

너무 넓지도, 과하게 조이지도 않는 소매핏으로 팔 라인이 자연스럽게 살아납니다.

### 3. 허리가 부해 보이지 않는 몸통 실루엣

가슴과 어깨는 살리고 몸통은 과하게 퍼지지 않도록 잡았습니다.

### 4. 전체 비율이 깔끔하게 떨어지는 기장감

짧게 뜨거나 길게 늘어지는 느낌을 줄여 데님, 슬랙스, 쇼츠와 균형 있게 매치됩니다.

## Model Fit Check

```text
MODEL 183cm / 78kg / L 착용
평소 상의 105 착용.
어깨와 가슴은 여유 있게 맞고, 팔 라인은 자연스럽게 잡히는 DORT 추천핏입니다.
```

## Recommended Customer

- 운동한 체형이 자연스럽게 드러나는 옷을 원하는 고객
- 너무 붙는 머슬핏이 부담스러운 고객
- 기본티를 입었을 때 몸통이 부해 보이는 것이 싫은 고객
- 상체 라인이 깔끔하게 보이는 데일리 티셔츠를 찾는 고객
- 하나의 티셔츠로 데님, 슬랙스, 쇼츠까지 돌려 입고 싶은 고객

## Size Guide

| Body Type / Usual Size | Recommended Size | Fit Feeling |
|---|---:|---|
| Slim-average / 95-100 | M | clean regular fit |
| Average / 100-105 | L | relaxed physical fit |
| Athletic / 105 | L | DORT recommended fit |
| Athletic or broad upper body / 105-110 | XL | comfortable physical fit |

## Detail Cut Plan

Required:

- Full-body front shot
- Upper-body fit shot
- Side shot showing chest and waist drape
- Back shot showing shoulder line
- Neckline detail
- Shoulder seam detail
- Sleeve hem detail
- Bottom hem detail
- Fabric touch shot

## Fabric Information

Soft cotton-blend fabric with slight stretch. Comfortable for repeated daily wear and suitable for spring, summer, and early fall.

Recommended display:

```text
Thickness: Medium
Stretch: Slight
Transparency: Low
Touch: Soft
Season: Spring / Summer / Early fall
```

## Styling Recommendations

### LOOK 01. 피지컬 데일리룩

쿨링 머슬 반팔티 + 롱 와이드 데님

상체는 탄탄하게, 전체 비율은 깔끔하게 잡아주는 기본 조합입니다.

### LOOK 02. 미니멀 데이트룩

쿨링 머슬 반팔티 + 세미와이드 슬랙스

깔끔하지만 몸 좋아 보이는 분위기를 만들고 싶을 때 추천합니다.

### LOOK 03. 여름 캐주얼룩

쿨링 머슬 반팔티 + 버뮤다 쇼츠

가볍지만 상체 라인이 무너지지 않는 여름 조합입니다.

## Purchase Check

- 타이트한 핏을 원하면 정사이즈를 추천합니다.
- 데일리하게 여유 있는 핏을 원하면 한 사이즈 업을 고려하세요.
- 어깨와 가슴이 발달한 체형은 상세 실측을 꼭 확인하세요.
- 건조기 사용 시 수축 가능성이 있어 자연 건조를 권장합니다.

## Full Page Order

```text
1. Hero image and product title
2. One-line positioning
3. Top 3 purchase hooks
4. Product summary
5. DORT FIT POINT
6. Model fit check
7. Full-body and upper-body fit photos
8. Detail cut section
9. Recommended customer
10. Size guide
11. Fabric information
12. Styling recommendations
13. Purchase check
14. Shipping / exchange notice
15. Reviews
```

## Final Copy Draft

운동한 체형을 자연스럽게 보여주는 데일리 피지컬핏 반팔티입니다.

어깨와 가슴 라인은 살리고, 허리와 몸통은 부해 보이지 않도록 잡았습니다. 과하게 붙는 헬스핏이 부담스러운 고객도 일상에서 편하게 입을 수 있는 균형감 있는 핏입니다.

하나만 입어도 상체 라인이 깔끔하게 살아나고, 데님과 슬랙스, 쇼츠까지 쉽게 매치됩니다.

## Missing Information

- Exact fabric composition
- Exact size measurements
- Real product photos
- Washing test result
- Customer review data
