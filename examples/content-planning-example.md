# Content Planning Example

## Input Summary

```text
Product name: [피지컬핏] 쿨링 머슬 반팔티
Product category: Short sleeve t-shirt
Product registration record: DORT-TEE-001-BK registration record, GREEN
Main fit benefit: Upper body looks cleaner and more defined without feeling like gym wear.
Main material/detail benefit: Soft cotton-blend fabric with slight stretch.
Recommended styling: Wide denim, semi-wide slacks, bermuda shorts.
Expected selling price: 37,900 KRW

Goal: Launch awareness and product page traffic
Platform: Instagram Reels, YouTube Shorts, Instagram Feed
Content type: Short-form videos and feed carousel posts
Posting period: 7 days

Available assets:
Model fit shots, upper-body close-up, detail cuts, fabric touch shot.

Target customer concern:
Customers worry that muscle-fit t-shirts are too tight, too gym-like, or make the waist look bulky.
```

## Output

# Content Planning Result

## Core Content Angle

```text
붙는 머슬핏이 부담스러운 남자를 위한 데일리 피지컬핏.
```

The product should be introduced as a daily t-shirt that makes the upper body look better without looking too tight or gym-focused.

## Hook Bank

1. 기본티 입었는데 몸이 부해 보인다면
2. 운동한 체형인데 티셔츠 핏이 애매하다면
3. 팔 라인은 살리고, 허리는 깔끔하게
4. 붙는 머슬핏이 부담스러울 때
5. 몸 좋아 보이는 기본티는 따로 있습니다
6. 티셔츠 하나로 상체 밸런스가 달라지는 이유
7. 데일리로 입기 좋은 피지컬핏 기준
8. 헬스복 같지 않은 머슬핏을 찾는다면

## Short-Form Video Ideas

### 1. 기본티 핏 실패 이유

Problem:

Generic t-shirts can make the body look boxy or flat.

Visuals:

- Front fit shot
- Upper-body close-up
- Waist line check

CTA:

```text
DORT 피지컬핏 반팔티
```

### 2. 붙는 머슬핏이 부담스러울 때

Problem:

Some muscle-fit shirts look too tight for dailywear.

Visuals:

- Sleeve fit
- Chest drape
- Side shot

CTA:

```text
운동한 체형은 살리고, 과한 느낌은 줄였습니다.
```

### 3. 티셔츠 하나로 3가지 코디

Problem:

Customers want one basic top that works with multiple bottoms.

Visuals:

- Denim look
- Slacks look
- Bermuda shorts look

CTA:

```text
하나로 돌려 입는 데일리 피지컬핏.
```

## Short-Form Script

Title:

```text
붙는 머슬핏이 부담스러울 때
```

Script:

```text
0-3s
붙는 머슬핏이 부담스럽다면

3-8s
운동한 체형을 보여주고 싶은데
너무 조이면 데일리로 입기 애매하죠.

8-15s
DORT 피지컬핏은
어깨와 팔 라인은 자연스럽게 살리고
몸통은 부해 보이지 않게 잡았습니다.

15-22s
데님에는 캐주얼하게,
슬랙스에는 깔끔하게,
쇼츠에는 여름 데일리룩으로.

22-30s
[피지컬핏] 쿨링 머슬 반팔티
데일리로 입는 몸 좋아 보이는 기본핏.
```

On-screen captions:

```text
붙는 머슬핏이 부담스러울 때
어깨는 넓게
팔은 자연스럽게
허리는 깔끔하게
데일리 피지컬핏
```

## Feed Carousel Ideas

### Carousel 1. 몸 좋아 보이는 기본티 기준

Slides:

1. 몸 좋아 보이는 기본티는 기준이 다릅니다
2. 어깨선이 애매하면 상체가 좁아 보입니다
3. 팔통이 너무 넓으면 팔 라인이 죽습니다
4. 허리가 퍼지면 몸이 부해 보입니다
5. DORT는 어깨, 팔, 허리 밸런스를 봅니다
6. 데일리로 입는 피지컬핏
7. `[피지컬핏] 쿨링 머슬 반팔티`

### Carousel 2. 헬스복 같지 않은 머슬핏

Slides:

1. 머슬핏이 부담스러운 이유
2. 너무 붙으면 일상에서 과해 보입니다
3. 너무 크면 체형이 묻힙니다
4. 중요한 건 붙는 정도가 아니라 균형입니다
5. 어깨와 팔은 살리고, 몸통은 정리하는 핏
6. DORT 피지컬핏
7. 상품 페이지에서 자세히 보기

## Feed Slide Copy

```text
Slide 1
몸 좋아 보이는 기본티는 기준이 다릅니다

Slide 2
어깨선이 무너지면 상체가 좁아 보입니다

Slide 3
팔통이 너무 넓으면 팔 라인이 죽습니다

Slide 4
허리가 퍼지면 전체 실루엣이 부해 보입니다

Slide 5
DORT는 어깨, 팔, 허리 밸런스를 기준으로 봅니다

Slide 6
운동한 체형은 자연스럽게 살리고
과하게 붙는 느낌은 줄였습니다

Slide 7
[피지컬핏] 쿨링 머슬 반팔티
```

## Caption Options

### Caption 1

```text
기본티는 단순해 보여도 핏 차이가 가장 크게 보이는 옷입니다.

어깨는 안정적으로,
팔은 자연스럽게,
허리는 부해 보이지 않게.

DORT가 생각하는 데일리 피지컬핏입니다.
```

### Caption 2

```text
붙는 머슬핏이 부담스럽다면,
데일리로 입을 수 있는 피지컬핏을 기준으로 보세요.

운동한 체형은 살리고
전체 실루엣은 깔끔하게 정리했습니다.
```

## Hashtags

```text
#DORT #도르트 #피지컬핏 #남자반팔티 #남자기본티 #남자데일리룩 #남자코디 #미니멀룩 #머슬핏반팔 #남자쇼핑몰
```

## Shooting Checklist

- Full-body front fit
- Upper-body close-up
- Sleeve fit close-up
- Waist line side shot
- Denim styling
- Slacks styling
- Bermuda shorts styling
- Fabric touch shot
- Walking shot for natural movement

## 7-Day Content Plan

| Day | Content | Purpose |
|---:|---|---|
| 1 | Reel: 붙는 머슬핏이 부담스러울 때 | Launch hook |
| 2 | Feed: 몸 좋아 보이는 기본티 기준 | Fit education |
| 3 | Short: 어깨/팔/허리 핏 체크 | Fit proof |
| 4 | Reel: 티셔츠 하나로 3가지 코디 | Styling |
| 5 | Feed: 헬스복 같지 않은 머슬핏 | Objection handling |
| 6 | Short: 원단 터치와 디테일컷 | Quality proof |
| 7 | Feed: DORT 피지컬핏 기준 | Brand identity |

## Reuse Map

```text
Detail page FIT POINT
→ Feed carousel slides
→ Reel captions
→ Product page hook
→ Customer size reply language
```

## Missing Information

- Real video clips
- Final product size measurements
- Exact fabric composition
- Launch promotion details
